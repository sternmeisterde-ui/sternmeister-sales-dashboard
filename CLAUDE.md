...
| `complaints` | Жалобы | manager(свои)+admin | `ComplaintsTab.tsx` | `/api/complaints` | D1 `complaints` (агрегат `evaluation_error_reports` daily-БД + `bug_reports`); снимки оценок заморожены в jsonb; bug_report линкуется к звонку по ссылке на сделку из текста. UI read-only: решения пишет ТОЛЬКО batch `/api/complaints/resolve` (Bearer) от OKK-адъюдикатора. См. `docs/DASHBOARD-ZHALOBY.md` |
