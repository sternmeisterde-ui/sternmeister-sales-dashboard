...
## «Потерянные» (B2B) — два вида потери

Плитка + drill-down (`getAnalyticsLostCallsDetail`, поле `kind`):

- `out_unanswered` — **мы звонили**: исходящий недозвон (duration < 1) в
  рабочие часы менеджера, по номеру никто не перезвонил в течение 15 минут.
  Скоуп по агентам (ростер + алиасы). Это была единственная ветка до 29.07.
- `in_missed` — **клиент звонил сам** (добавлено 2026-07-29 по жалобе РОПа
  «в потерянных нет входящих»): входящий на линию `KOM%` в рабочие часы
  отдела, никто не взял (duration < 1) и не перезвонили в течение 24 часов.
  Входящие Коммерсов падают в очередь БЕЗ агента (`manager IS NULL`), поэтому
  скоуп по `line_name`, а не по ростеру; в drill-down такие строки идут
  отдельной группой «Входящие — никто не взял» с бейджем «входящий».

**Рабочие часы — из ГРАФИКА, а не хардкод** (2026-07-29, решение владельца;
до этого у всех было 09:00–19:00, хотя смены разные: 09–18 / 10–19 / 11–20).
Источник — `manager_schedule` (файл РОПа), поля `shift_start_time` /
`shift_end_time`; helper `getShiftWindows(dept, from, to)` в
`src/lib/db/queries-daily.ts`. График лежит в D1, звонки — в Analytics,
поэтому окно применяется в TS после SQL (в выдаче есть `berlin_date` /
`berlin_min`). Правила:

1. есть смена у этого менеджера в этот день → его окно;
2. нет его смены, но кто-то был на линии → дневное окно отдела
   (мин. начало … макс. конец) — звонок вне графика не исчезает из метрики;
3. графика на дату нет вообще (месяц не импортирован) → фолбэк 09:00–19:00,
   чтобы метрика не обнулилась молча. ⚠ У Госников графика в
   `manager_schedule` нет — они всегда на фолбэке (их плитка «Потерянные»
   всё равно лид-based, эта функция кормит только `perManager.lostCalls`).

Пропущенный входящий к менеджеру не привязать (очередь без агента) → для него
всегда дневное окно отдела; если в этот день никто не работал (выходной) —
это не потеря, строка отбрасывается.

Порядок величин (27.07.2026): 59 недозвонов + 1 пропущенный входящий = 60
(было 64 при окне 9–19: 4 звонка сделаны после конца смены).
Входящих у Коммерсов единицы в день — метрика почти не меняется по числу,
но закрывает главный вопрос «а где клиенты, которые звонили нам сами».

У Госников «Потерянные» — другая, лид-based метрика (`getB2gLostLeads`,
спека 25 §2); ветка входящих туда НЕ добавлена.
