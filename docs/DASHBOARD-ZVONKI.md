...
**Из рабочего времени вычитается личная занятость автора первого касания**
(решение РОПа 2026-08-04, вариант «А»):

- отмеченные им статусы **Обед / Встреча / Обучение** — лента присутствия CloudTalk
  (`cloudtalk_status_intervals`, та же, что рисует дорожку в «Активности»);
- **разговоры с другими клиентами** — от начала набора до конца разговора
  (гудки + разговор), как «в телефоне» в «Активности».

«Перерыв» и «Занят» НЕ вычитаются: в «Активности» они считаются простоем, и метрики
не должны расходиться между вкладками. Вычитается только пересечение занятости с
рабочим окном — обед за пределами смены не срезает время дважды
(`scheduleBusinessWindows` + `workingSecondsMinusBusy` в `business-hours.ts`).

⚠️ **Лента статусов собирается только с 27.07.2026** — за более ранние периоды
вычета статусов нет. Вместе с тем, что график смен есть только с августа, это значит:
июнь–июль и август посчитаны по разным правилам и напрямую несопоставимы.
...
Исходящие CloudTalk помечены каналом набора — **дайлер** или **вручную**
(`analytics.dialer_call_attribution`, ключ `communication_id = 'ct:' || cdr_id`).
Метка ставится **только по строке атрибуции** — никаких домыслов. Её нет, если вопрос
неприменим (CallGear/WhatsApp, входящие — атрибуция покрывает лишь исходящие) или если
строки нет: до 2026-07-01 (горизонт бэкфилла dialer-sync) и пока часовой синк не разобрал
свежие звонки.

Это намеренно строже дайлер-вида «Активности», где всё неатрибутированное считается
«вручную». В таблице есть и ЯВНЫЕ manual-строки (23170 против 2043 dialer), поэтому
отсутствие строки значит «ещё не разобрано», а не «набрал руками» — и если dialer-sync
встанет, свежие звонки в пер-звонковом списке честно покажут прочерк вместо вранья.
Покрытие на 2026-08-03 — 100% за последние 10 дней.
