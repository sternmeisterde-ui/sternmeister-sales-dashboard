...
колонки-дни окна, все метрики упакованы в одну ячейку — сверху звонки,
снизу мелко дозвон (зелёный) · пропущенные (красный), легенда под таблицей
(итерации фидбека 2026-07-28: пилюли → строки-метрики → компактные ячейки);
«Итого» первой колонкой, футер «Всего», выходные из `manager_schedule`
серым, тултип ячейки — все метрики за день с подписями.
