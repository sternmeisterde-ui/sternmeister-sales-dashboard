...
Направления (b2g): каждая строка аннотируется `managerLine` — текущей линией
менеджера-субъекта из `master_managers.line` (1=Квалификатор, 2=Бератеры,
3=Доведение). Фильтр-чипы линий во вкладке — клиентские (реестр мал);
несмэтченные строки видны только в «Все линии».
