...
Переделка секции «Звонки» 2026-07-22 (ТЗ Рузанны):
- «Менеджеров на линии план» и «Замороженные лиды (без первого звонка)» — убраны;
- «Менеджеров на линии» (бывш. «…факт») — из Графика (`manager_schedule` ←
  Google-файл РОПа через `sync-b2b-schedule`): day = on_line на дату,
  week/month = unique on_line за период (`getUniqueOnLineManagerCount`);
- план ожидания ответа = константа 30, план SLA = константа 25 (не
  редактируются, отдаются из `getB2BFact`, из `B2B_FIXED_PLAN_DEFAULTS` удалены);
- факты ожидания и SLA — В МИНУТАХ (число с 1 знаком, не HH:MM:SS):
  `calls_sla_f` конвертируется из секунд на выдаче; `calls_avgWait_f` — ручной
  ввод, значение трактуется как минуты. ⚠ Исторический ручной ввод
  `calls_avgWait_f` (янв–апр 2026, значения ~23–42) делался в СЕКУНДАХ и не
  конвертирован — старые даты читаются как «минуты». Ключи `calls_sla_f/p`,
  `calls_avgWait_f/p` исключены из DURATION_SEC/MIN_KEYS (DailyTab) и
  FACT_KEYS_IN_SECONDS/PLAN_KEYS_IN_MINUTES (build-response) — у Гос всё
  по-старому (секунды + HH:MM:SS).
