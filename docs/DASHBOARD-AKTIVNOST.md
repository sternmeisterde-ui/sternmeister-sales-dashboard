...
## Обновления 2026-07-22 (задачи Лилии, b2g)

1. **«В телефоне» vs «в диалоге» (b2g).** Синее время = разговор + дозвон
   исходящих (`communications.wait_seconds`); недозвоны тоже красятся синим
   («Недозвон · гудки Nс»). Чистый разговор — отдельно `minutes.talk`
   («в диалоге» в сводке/плитке). Пропущенные ВХОДЯЩИЕ активностью не считаются.
   b2b не тронут (waitSec=0, durationExpr-семантика прежняя).
2. **Ручные статусы менеджеров (b2g)**: обед / встреча / завершил день.
   Таблица `manager_status_intervals` (Tracking DB, bootstrap в init.ts),
   API `/api/tracking/status` (менеджер — только «в моменте»; админ — add/delete
   задним числом, время naive = Berlin). Красятся ПОВЕРХ простоя (звонок/CRM
   выигрывают), обед вычитается из простоя максимум 60 мин/день, встречи —
   целиком, «завершил день» — только визуал. Плашка «Мой статус» в шапке
   (менеджерская сессия), админ-правка — в лупе дня (секция «Статусы дня»).
3. **Сводная таблица «Итоги за период»** под общим видом: пофамильно —
   раб. дней, в телефоне, в диалоге, CRM, обед, встречи, простой, утилизация %.
   Считается на клиенте из days[].minutes.
4. **«Кампании дайлера»** в дайлер-виде: плитки per campaign (звонков, дозвон %,
   разговор, средний) из `analytics.dialer_call_attribution` за период
   (в ответе `/api/tracking/dialer-leads`).
5. **Фикс модалки общего вида**: с tracking v12 в `tracking_events` нет звонков —
   модалка рисовала таймлайн без синего. Теперь detail-роут подмешивает звонки
   из analytics (как главный вид).
