...
function processBlocks(
  ...
  for (const rawBlock of blocks) {
    ...
    for (const rawC of criteria) {
      ...
      // Унаследованный вердикт (OKK verdict-inheritance, с 2026-07-07):
      // call_type-исключённый критерий followup-звонка несёт КОПИЮ вердикта
      // из прошлого звонка клиента (inherited_from_call_id). В балл звонка он
      // входит по решению владельца, но в покритериальную среднюю менеджера
      // не должен: один реальный вердикт первого звонка иначе считался бы
      // повторно в каждом followup (1 приветствие × N звонков = N-кратный
      // вес в колонке «Приветствие»).
      if (c.inherited_from_call_id) continue;
