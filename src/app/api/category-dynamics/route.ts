...
export async function GET(request: NextRequest) {
  try {
    ...
    // Границы передаются в SQL СТРОКАМИ civil-дат и сравниваются по
    // берлинскому календарю — Date-параметры здесь запрещены (сдвиг окна
    // на TZ сервера, баг «87 vs 88», см. data.ts).
    const dims = await getCategoryDynamicsDays(funnel, fromStr, toStr);
