...
// ✅ Подтверждено перекрёстной проверкой (2026-07-07): в данных интегратора
// все false ≤ 14 звонков, все true ≥ 27, зазор 15–26 пуст — порог 20 на
// реальных данных неотличим от любого другого из зазора.
export const TOUCH_IGNORE_MIN_CALLS = 20;
...
/**
 * Норматив SLA первого звонка (сек от начала смены) — НАША метрика.
 * Формула сводного «SLA, %» интегратора не воспроизводима: перекрёстная
 * проверка (23a §4, 2026-07-07) опровергла обе правдоподобные версии —
 * «доля ≤ порога времени» (✗ июнь) и «доля лидов, которым позвонили»
 * (✗ июль); их формула противоречит сама себе между месяцами. Осознанное
 * расхождение с Looker: 10 минут от начала смены — понятная и защитимая
 * норма (нижний бакет вкладки Looker). Порог может пересмотреть РОП.
 */
export const SLA_TARGET_SECONDS = 600;
