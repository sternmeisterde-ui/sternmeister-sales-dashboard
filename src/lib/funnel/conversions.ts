...
export function conversionNote(id: ConversionId): ConversionNote | null {
  if (id === "C3.1") {
    return {
      ...
      points: [
        "Из лидов, у кого Термин ДЦ реально состоялся, — какая доля продвинулась дальше в АА.",
        "Продвинулись = консультация АА / на рассмотрении / отложенный старт / апелляция / Гутшайн. «Термин АА отменён/перенесён» продвижением НЕ считается.",
        "Потеря (Отсев) = застрял на «Термин ДЦ» ИЛИ сразу ушёл в «Закрыто» без АА. Застрявшие на ДЦ ВХОДЯТ в знаменатель.",
        "⚠ База — только лиды с проставленным статусом «Термин ДЦ». Самые свежие недели «застрявших» завышены — лиды ещё в полёте (окно зрелости это сглаживает).",
      ],
    };
  }
  return null;
