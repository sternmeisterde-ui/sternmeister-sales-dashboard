        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
            <Tile
              ...
              hint="Соединённые звонки бератера от 10 минут, сделанные пока сделка стояла на этапе консультации перед ДЦ или АА. Считается по телефонии, а не по ОКК, поэтому ничего не теряется. Оборвавшийся разговор считается один раз, а не двумя звонками."
            />
            <Tile
              label="Разобрано ОКК"
              value={data.totals.analyzed}
              sub={data.coveragePct === null ? undefined : `${data.coveragePct}% консультаций`}
              hint="Порог разбора — 10 минут, как и порог консультации; июль пересчитан целиком, раньше система брала от 15 минут. Разбирается всё равно не всё: повторный звонок к сделке с 4+ касаниями, уехавший этап, сбой обработки, не дошедшая запись. Причина по каждому клиенту — в подсказке к его строке. Неразобранное ≠ «ролевки не было»."
            />
            <Tile
            ...
            />
            <Tile
              label="Оценок выставлено"
              value={data.totals.botScores}
              sub={
                data.totals.confirmed > 0
                  ? `из ${data.totals.confirmed} подтверждённых`
                  : undefined
              }
              hint="Сколько ролевок бот реально оценил баллом 1–5 — именно эти цифры уезжают в карточки Kommo. Подтверждённая репетиция без балла (в таблице ниже — знак ○) сюда не входит: материала для оценки не хватило."
            />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-slate-500">
              ...
              <Th sub="звонки от 10 минут" title="Соединённые звонки БЕРАТЕРА не короче 10 минут, сделанные пока сделка стояла на этапе консультации перед ДЦ или АА. Звонки квалификатора и доведения сюда не входят — ролевку ведёт бератер. Считается по телефонии, ничего не теряется; оборвавшийся разговор считается один раз.">
                Консультаций
              </Th>
              ...
              <Th sub="из консультаций" title="Считается в звонках, как и «Консультаций». Оборвавшийся разговор (связь упала, менеджер перезвонил) бот оценивает целиком и считается он один раз. Разбирается не всё: повторный звонок к сделке с 4+ касаниями, уехавший этап, сбой обработки, не дошедшая запись. Причина по каждому клиенту — в таблице ниже. Неразобранное не значит «ролевки не было».">
                Разобрал ОКК
              </Th>
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-slate-900/80 backdrop-blur z-10">
            <tr className="text-slate-500">
              ...
              <Th align="center" sub="1–5, по порядку" title="Балл клиента за ролевку — по одному значку на разобранный разговор. Цифра — выставленный балл; ○ — репетиция была, но материала на оценку не хватило; · — бот разобрал, репетиции не было; ⚠ — сбой авто-оценки. В счётчик «Оценок выставлено» идут только цифры.">
                Оценил бот
              </Th>
                        ...
                        <span key={i} className="inline-flex items-center gap-1">
                          ...
                          {b.score !== null ? (
                            ...
                            <span
                              ...
                              title="Репетиция была, но клиент дал слишком мало самостоятельных немецких ответов — балл не выставлен"
                            >
                              ○
                            </span>
                          ) : (
                            // Бот разобрал разговор и репетиции в нём не нашёл. Раньше
                            // такие показывались тем же ○ с подписью «ролевка была»,
                            // из-за чего колонка выглядела полнее, чем есть: за июль
                            // 137 таких знаков против 183 реальных баллов.
                            <span
                              className="text-slate-700"
                              title="Бот разобрал этот разговор: репетиции в нём не было"
                            >
                              ·
                            </span>
                          )}
