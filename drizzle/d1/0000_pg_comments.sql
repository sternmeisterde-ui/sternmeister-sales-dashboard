...
COMMENT ON COLUMN public.daily_plans.department IS '''b2g'' | ''b2b''.';
COMMENT ON COLUMN public.daily_plans.line IS 'B2G: ''1'' (квалификатор), ''2'' (бератер), ''3'' (доведение), ''funnel'' (общая воронка). B2B: ''salesTotal'' / ''salesBuh'' / ''salesMed'' / ''calls''.';
COMMENT ON COLUMN public.daily_plans.user_id IS 'NULL = line-level дефолт (применяется ко всем менеджерам линии). Иначе → master_managers.id (per-manager override).';
COMMENT ON COLUMN public.daily_plans.metric_key IS 'Имя метрики из metrics-config.ts (qualLeads, payments, calls, etc).';
COMMENT ON COLUMN public.daily_plans.plan_value IS 'Целевое значение (text, парсится consumer''ом).';
COMMENT ON COLUMN public.daily_plans.period_type IS '''day'' | ''week'' | ''month''.';
COMMENT ON COLUMN public.daily_plans.period_date IS '''YYYY-MM-DD'' (day) | ''YYYY-WNN'' (week) | ''YYYY-MM'' (month).';
...
COMMENT ON COLUMN public.manager_bonuses.user_id IS 'FK → master_managers.id.';
COMMENT ON COLUMN public.manager_bonuses.period_month IS '''YYYY-MM'' Berlin civil-month. Composite UNIQUE (user_id, period_month).';
COMMENT ON COLUMN public.manager_bonuses.amount IS 'Положительная сумма премии (или 0 = нет; в БД мы удаляем строку при amount=0).';
COMMENT ON COLUMN public.manager_bonuses.note IS 'Опциональное «за что» — freeform.';
...
-- ─── public.r1_avatars ───
COMMENT ON TABLE public.r1_avatars IS '[D1 / B2G + общее] AI-аватары для B2B ролевок (та же схема что d1_avatars, но физически в R1 БД).';

-- ─── public.r1_calls ───
COMMENT ON TABLE public.r1_calls IS '[D1 / B2G + общее] AI-ролевки B2B: та же схема что d1_calls. user_id → r1_users; avatar_id → r1_avatars. Used by tabs: Дейли.';

-- ─── public.r1_users ───
COMMENT ON TABLE public.r1_users IS '[D1 / B2G + общее] Ролевочные пользователи B2B (зеркало master_managers с in_rolevki=true, department=''b2b''). team=''ruzanna''.';
